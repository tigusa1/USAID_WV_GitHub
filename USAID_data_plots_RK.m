...
flag.old_data    = 2; % 2-old, 1-new, old scaling, 0-new, new scaling (also changes lambda)
flag.correct     = 1; % 1-change sign in GM, 0-original sign
...
if flag.old_data==2
    T = readtable('data_2021_09_07.xlsx','sheet',country); % ver (07a) has data centered at 0.5
    yrs_max = 2021; % 2021 is the last year for old data
else
    if flag.old_data==1
        T = readtable('data_2022_11_26aTI_raw.xlsx','sheet',country);
        T = readtable('data_2022_11_26bTI_raw.xlsx','sheet',country); % ver (26c): centered at 0.5
        yrs_max = 2022;
    else
...
warn = warning('query','last'); warning('off',warn.identifier);
...
data_values = init_variables(data_values_orig,years_N,var_N,var_names_orig,stocks_idx,...
    yrs,var_names);
...
if flag.old_data==1
    if strcmp(country,'ES')
        ...
        lambda = 0.001*.9*6*3.5;  %3.5; % 1 (small betas ~ 0.7), 2 (0.1), 1.85
%       lambda = 0.001*.9*6*1.75; %3.5; % 1 (small betas ~ 0.7), 2 (0.1), 1.85
    end
elseif flag.old_data==2
    if strcmp(country,'ES')
        lambda = 0.001*.9*6*1;  flag_lasso = 0;
    else
        lambda = 0.001*.9*6*1.75; % 1 (small betas ~ 0.7), 2 (0.1), 1.85
    end
else
...
% Alternative: Metropolis-adjusted Langevin algorithm
flag.ABC = 1;
if flag.ABC
    N_ABC = 100*100*9;
    for i=1:N_ABC
        betas_i(i,:) = abs(randn(size(betas0)))*1.5;
...
%           idx_2022 = [ 2 4 7 15 16  17 18 26 27 28 32  41:50 52];
            idx_2022 = [ 2 4 7 15 16  17 18 26 27 28];               % betaSE(32) makes LE->IN large
%           idx      = setdiff(union(idx_2021,idx_2022),19);
        ...
        [betasSqrtIdx,~,~,~,~,hessianSqrtSelect] = fminunc(@(betas_sqrt_idx) ...
            ...
            lambda,flag_lasso),betasSqrt(idx),options);

        se = sqrt(diag(inv(hessianSqrtSelect)));
        betasSqrtSESelect = betasSqrt; betasSqrtSESelect(idx) = betasSqrt(idx) + se;
...
% SET STOCKS AT TIME t+1
    ...
    if flag.RK
        Z1 = fDZ(Z_k,betas,...
            ...
            Gang_Affiliation,Gang_Cohesion,Gang_Control,Interventions,Impunity_Governance,Machismo,...
            Mental_Health,Neighborhood_Stigma,School_Quality,Territorial_Fights,Victimizer,Youth_Empowerment,...
            beta_N,var_S_N,machismo_int,youth_int,stigma_int,f_breakdown_int,mh_int,f_cohesion_int,...
            H_int_1,H_int_2,resilience,flag);
        Z2 = fDZ(Z_k + Z1*dt/2,betas,...
            Access_to_Abortion,Access_to_Contraception,Bad_Governance,Bully,Deportation,Economy,...
            Economy_Opportunities,Exposure_to_Violent_Media,Extortion,Family_Breakdown,Family_Cohesion,...
            Gang_Affiliation,Gang_Cohesion,Gang_Control,Interventions,Impunity_Governance,Machismo,...
            Mental_Health,Neighborhood_Stigma,School_Quality,Territorial_Fights,Victimizer,Youth_Empowerment,...
...
S_PHV=Z(1); S_LE=Z(2); S_GM=Z(3); S_IN =Z(4);  S_PG=Z(5);  S_SD=Z(6);
S_UN =Z(7); S_SV=Z(8); S_TM=Z(9); S_PSV=Z(10); S_SA=Z(11); S_MD=Z(12);
