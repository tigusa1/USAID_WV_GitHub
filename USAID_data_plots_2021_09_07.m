function USAID_data_plots

global ts i_beta a_beta var_S_N beta_N i_goods pcolors
global machismo_int youth_int stigma_int f_breakdown_int mh_int f_cohesion_int
...
flag_intervention = 2; % 0-none, 1-all interventions, 2-direct injection
country = 'ES'; % H or ES
...
yrs_max = 2021; % 2021 is the last year
...
global t_num dt ts t_N y_data_ts weights_ts i_goods stock_initial i_beta a_beta
    ...
    for i=1:var_N
        if i<=var_S_N
            ...
            if flag_eval
                ...
                Bad_Governance=Bad_Governance_all(t_int);
                Bully=Bully_all(t_int);
                ...
                Gang_Affiliation=Gang_Affiliation_all(t_int);
                Gang_Cohesion=Gang_Cohesion_all(t_int);
                ...
                Territorial_Fights=Territorial_Fights_all(t_int);
                Victimizer=Victimizer_all(t_int);
                Youth_Empowerment=Youth_Empowerment_all(t_int);
            end
        end
    end
    betas(i_beta) = Betas;
...
% PLOT MODEL RESULTS
fig = figure(220+fig_no); fig.Name = 'compare data'; clf; fig.Position = [680 263 1014 714];
for i=1:var_S_N
    subplot(3,4,i)
    idx = stocks_idx(i);
    S_var_name_all = strcat("S_",var_names_orig(idx),"_all");
    eval(strcat("plot(yrs,",S_var_name_all,",'-','LineWidth',2)"))
    hold on, plot(yrs,y_data(:,i),'LineWidth',2), hold off
    title(var_names(idx)), ax = gca; ax.XLim = [1 years_N]; ax.XTick = [1 years_N];
    ax.XTickLabel = yrs_lbl; ax.YLim = [-.5 1.5]; % can be negative
end
